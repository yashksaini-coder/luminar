...
from backend.gossip.scoring import PeerScoreTracker, PeerScoreParams
...
async def test_heartbeat_ihave_iwant(event_bus, clock):
    ...
    edges = [(f"peer-0", f"peer-{i}") for i in range(1, n)]
...
async def test_heartbeat_prunes_low_score_peers(gossip, event_bus, clock):
    ...
    prune_events = [e for e in event_bus.ring if e.event_type == "GossipPrune" and e.to_peer == "peer-1"]
...
async def test_heartbeat_grafts_high_score_peers_first(gossip, event_bus, clock):
    ...
    graft_events = [e for e in event_bus.ring if e.event_type == "GossipGraft" and e.from_peer == "peer-0"]
